...
def shortDesc():
    return "（直播）帧率／码率不变，检测分辨率先降后升"
def detailDesc():
    return "设备A/B以主播方式进入频道，设备A设置分辨率先降后升，检测分辨率升降情况"
def run():
    ...
    lib.ExeCmdCallBack(0,"joinChannelByKey,nil,Test00215,nil,1") # uid:1
    ...
    lib.ExeCmdCallBack(1,"joinChannelByKey,nil,Test00215,nil,2") # uid:2
    time.sleep(10)
    ...
    if width2 == 1280 and height2 == 720:
        return "0"
    else:
        return "-1"
